#!/usr/bin/env python3
"""
Scale the validated single-subject pipeline to multiple ltpFR2 subjects.

For N distinct subjects it selects the smallest VALID ltpFR2 session
(task==ltpFR2, WORD==576, 576/576 peers coverage, 576/576 valid 300-800 ms
EEG windows — screened cheaply from the EDF header before downloading), then
per session runs the exact validated chain:

    download_one_session.py  (--task ltpFR2, sidecars + one EDF only)
    create_encoding_trials.py  -> encoding_trials.csv (recall from REC_WORD/item_num)
    build_trial_targets.py     -> Y_t5.npy (validated PEERS T5 embeddings)
    extract_eeg_features.py    -> X_eeg.npy (sample-based 300-800 ms, raw 500 Hz)
    ridge_corrected_metrics.py -> fidelity_results_corrected.csv (+shuffled control)
...
import find_ltpFR2_sessions as F  # noqa: E402
...
def process_session(sub, ses, task, log):
    ...
    for cmd, name in steps:
        rc = run(cmd, log)
        # create_encoding_trials exits 2 only if trials dropped; for screened
