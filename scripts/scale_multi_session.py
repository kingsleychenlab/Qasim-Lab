#!/usr/bin/env python3
"""
Extend the validated pipeline to MULTIPLE valid ltpFR2 sessions PER SUBJECT, so
the final model can include a session term.

Reuses the exact per-session chain from scale_multi_subject.process_session
(download -> encoding_trials -> Y_t5 -> X_eeg -> corrected ridge metrics) with
NO change to the core project logic. Only the SELECTION differs: for each of
N subjects it takes up to K valid sessions (validity screened cheaply from the
EDF header before download).

A session is valid only if: task==ltpFR2, WORD==576, peers coverage 576/576,
and 576/576 valid 300-800 ms EEG windows.

Combined table -> outputs/all_sessions_fidelity_results.csv with:
...
where embedding_fidelity == raw_cosine (matches the original project outline).

Does NOT run the final mixed-effects model.
"""
...
import scale_multi_subject as M            # noqa: E402
