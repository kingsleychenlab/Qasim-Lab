#!/usr/bin/env python3
"""
Download ONE subject/session of ds004395 (PEERS) from OpenNeuro's public S3
bucket, using anonymous HTTPS requests. This deliberately grabs a single
session (~500 MB EDF + tiny sidecars), NOT the full 8.7 TB dataset.

The dataset is a public OpenNeuro mirror on S3:
    https://s3.amazonaws.com/openneuro.org/ds004395/...

Usage:
    python scripts/download_one_session.py --sub LTP063 --ses 15
    python scripts/download_one_session.py --list-subject LTP063   # list sessions+sizes, no download
    python scripts/download_one_session.py --sub LTP063 --ses 15 --skip-eeg  # sidecars only
"""
