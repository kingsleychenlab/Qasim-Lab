...
class Tee:
    """Write to stdout and the summary file simultaneously."""
    ...
    def check(self, label, ok, detail=""):
        ...
        if not ok:
            self.errors += 1
        self.__call__(f"[{tag}] {label}" + (f" -- {detail}" if detail else ""))

    def warn(self, label, detail=""):
        self.warnings += 1
        self.__call__(f"[WARN] {label}" + (f" -- {detail}" if detail else ""))
...
def main():
    ...
    with open(args.out_summary, "w") as fh:
        ...
        # -----------------------------------------------------------------
        # Build recall lookup: set of (trial, item_num) recalled
        # and (trial, item_num) -> item_name for backup validation.
        # -----------------------------------------------------------------
        ...
        # -----------------------------------------------------------------
        # serialpos: order WORD events within each trial by onset (1-based).
        ...
        log(f"errors: {log.errors}   warnings: {log.warnings}")
        if log.errors:
