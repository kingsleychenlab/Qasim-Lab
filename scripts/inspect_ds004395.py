...
Usage:
    python scripts/inspect_ds004395.py
    python scripts/inspect_ds004395.py --data-root data/ds004395 --out outputs/ds004395_inspection_report.txt
"""
...
def main():
    ...
    with open(args.out, "w") as fh:
        ...
        if not os.path.isdir(args.data_root):
            ...
            log("  python scripts/download_one_session.py --sub LTP063 --ses 15")
            sys.exit(1)
        ...
        if eeg_files:
            ...
            log("No EEG binary present locally (sidecars only?). "
                "Download one with scripts/download_one_session.py to read it with MNE.")
