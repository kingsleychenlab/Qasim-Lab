#!/usr/bin/env python3
"""
Build the results/ package: summary CSV tables + figures.

READ-ONLY w.r.t. the pipeline: reads from outputs/ and project root, writes only
into results/ and results/figures/. Does not modify any existing output.
"""
...
sss.to_csv(os.path.join(RES, "summary_subjects_sessions.csv"), index=False)
created.append("results/summary_subjects_sessions.csv")
...
for c in rvf.columns:
    if c not in ("group", "n"):
        rvf[c] = rvf[c].round(5)
rvf.to_csv(os.path.join(RES, "remembered_vs_forgotten_summary.csv"), index=False)
created.append("results/remembered_vs_forgotten_summary.csv")
...
fmt.to_csv(os.path.join(RES, "final_model_table.csv"), index=False)
created.append("results/final_model_table.csv")
