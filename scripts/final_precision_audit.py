#!/usr/bin/env python3
"""
Full precision audit of the Neurolab project. Read-only; verifies every claim
against source files. Writes findings to outputs/final_precision_audit.txt.

Covers: structure, T5 numeric integrity + metadata, session validity, recall
re-derivation from events, EEG re-extraction from EDF, ridge/decoding config,
corrected-metric ranges, final-model numbers, gitignore. (T5 recompute from the
model and the results-wording scan are appended by separate steps.)
"""
...
# --- 1. structure ---------------------------------------------------
log("\n## 1. STRUCTURE")
req = ["peers_words.csv", "peers_word_order.csv", "peers_t5large_embeddings.npy",
       ...
       "results/pipeline_summary.md", "results/audit_report.md",
       "results/final_results.md", "results/portfolio_summary.md",
...
words = pd.read_csv(os.path.join(HERE, "peers_words.csv"))
order = pd.read_csv(os.path.join(HERE, "peers_word_order.csv"))
emb = np.load(os.path.join(HERE, "peers_t5large_embeddings.npy"))
meta = json.load(open(os.path.join(HERE, "embedding_metadata.json")))
...
src = open(os.path.join(HERE, "scripts/ridge_corrected_metrics.py")).read()
ck("ridge", "scaler fit on train fold only (source)",
   "StandardScaler().fit(X[tr])" in src, "StandardScaler().fit(X[tr])",
   "scripts/ridge_corrected_metrics.py")
...
ck("ridge", "raw_cosine defined as cos(pred,true) (source)",
   "corr = s[ci]" in src and "cosine_matrix" in src, "cosine to correct candidate",
   "scripts/ridge_corrected_metrics.py")
...
fsrc = open(os.path.join(HERE, "scripts/run_final_memory_model.py")).read()
ck("final", "logistic mixed-effects (BinomialBayesMixedGLM) in source",
   "BinomialBayesMixedGLM" in fsrc, "GLMM present", "scripts/run_final_memory_model.py")
ck("final", "supplementary metrics labeled SUPPLEMENTARY",
   ...
   "supplementary list", "scripts/run_final_memory_model.py")

# --- 11. gitignore --------------------------------------------------
log("\n## 11. GITIGNORE")
gi_path = os.path.join(HERE, ".gitignore")
gi = open(gi_path).read() if os.path.isfile(gi_path) else ""
needed = ["data/", "*.edf", "*.bdf", "outputs/subjects/", "outputs/X_eeg.npy",
          "outputs/Y_t5.npy", "predicted_embeddings", "venv/", "__pycache__", ".DS_Store"]
for pat in needed:
    ck("gitignore", f"ignores {pat}", pat in gi, "present" if pat in gi else "ABSENT", ".gitignore")
...
with open(OUT, "w") as f:
    f.write("\n".join(lines) + "\n")
print(f"\nwrote {OUT}")
print(f"wrote outputs/final_precision_audit_checks.csv")
